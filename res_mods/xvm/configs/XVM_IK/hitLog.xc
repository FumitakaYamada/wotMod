 ...
 */
{
  "hitLog": {
    ...
    "hpLeft": {
        ...
        "header": "<font color='#FCFCFC'>{{l10n:hpLeftTitle}}</font>",
        // Row in HP list (macros allowed, see readme-en.txt).
        // Строка в списке попаданий (допускаются макроподстановки, см. readme-ru.txt)
        "format": "<textformat leading='-4' tabstops='[50,90,170]'><font color='{{c:hp-ratio}}'>     {{hp}}</font><tab><font color='#FCFCFC'>/ </font>{{hp-max}}<tab><font color='#FCFCFC'>| </font><font color='{{c:vtype}}'>{{vehicle}}</font><tab><font color='#FCFCFC'>| {{nick}}</font></textformat>"
    },
    ...
    "defaultHeader":  "<font color='#FCFCFC'>{{l10n:Hits}}:</font> <font size='13'>#0</font>",
    ...
    "formatHeader":  "<font color='#FCFCFC'>{{l10n:Last}}:</font> <font color='{{c:dmg-kind}}'><b>{{dmg}}</b></font> {{dead}} <font color='#FCFCFC'>{{l10n:Total}}: </font><b>{{dmg-total}}</b>  <font color='#FCFCFC'>{{l10n:Hits}}:</font> <font size='13'>#{{n}}</font>",
    // List of hits format (macros allowed, see readme-en.txt).
    // Формат лога попаданий (допускаются макроподстановки, см. readme-ru.txt)
    "formatHistory": "<textformat leading='-4' tabstops='[30,50,85,110,190]'><font color='{{c:dmg-kind}}'>{{dmg}}<tab>{{dead}}</font><tab>| {{dmg-player}}<tab><font size='12'>\u00D7{{n-player}}</font><tab>| <font color='{{c:vtype}}'>{{vehicle}}</font><tab><font color='#FCFCFC'>| {{nick}}</font></textformat>",
    // Shadow options.
    // Параметры тени.
    "shadow": {
      // Opacity.
